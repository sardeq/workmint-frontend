    ...
    return (
      <div className="wm-rail-item" key={milestone.id}>
        ...
        <div className={`wm-milestone ${awaitingYou ? 'wm-milestone--live' : ''} ${milestone.status === 'pending' ? 'wm-milestone--pending' : ''}`}>
          ...
          {/* your own revision note, still open */}
          {milestone.status === 'revision' && (
            <div className="wm-note wm-note--danger">
              ...
              <div className="text-muted mt-2" style={{ fontSize: '0.82rem' }}>
                Waiting on {order.freelancer.name} to resend.
              </div>
            </div>
          )}
            ...
            <Form onSubmit={(e) => submitRevision(e, milestone)} className="mt-3">
              ...
              <div className="d-flex justify-content-between align-items-center">
                <span className="text-muted" style={{ fontSize: '0.8rem' }}>
                  This counts as revision {milestone.revisionsUsed + 1} of {order.revisionsIncluded}.
                </span>
                <div className="d-flex gap-2">
      ...
      {tab === 'messages' && (
        <div className="wm-panel">
          {order.messages.length === 0 ? (
            <EmptyState icon="chat" title="No messages yet" body={`Say hello to ${order.freelancer.name}.`} />
          ) : (
            <div className="wm-chat">
              {order.messages.map((m) => (
                <div key={m.id} className={`wm-bubble wm-bubble--${m.from === 'client' ? 'you' : 'client'}`}>
                  {m.text}
                  <div className="wm-bubble__meta">
                    {m.from === 'client' ? 'You' : order.freelancer.name} &middot; {timeAgo(m.at)}
                  </div>
                </div>
              ))}
              <div ref={chatEndRef} />
          ...
          <Form onSubmit={handleSend} className="d-flex gap-2 mt-3 pt-3 border-top">
            <Form.Control
              placeholder={`Message ${order.freelancer.name}`}
              value={message}
            ...
            order.changeRequests.map((cr) => (
              <div key={cr.id} className="py-3 border-bottom">
                <div className="d-flex justify-content-between align-items-start gap-3">
                  <div>
                    ...
                    <div className="text-muted" style={{ fontSize: '0.8rem' }}>
                      From {order.freelancer.name} &middot; {timeAgo(cr.at)}
                    </div>
                  </div>
                ...
                {cr.status === 'Pending' && (
                  <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 mt-3">
                    <span className="text-muted" style={{ fontSize: '0.82rem' }}>
                      Approving funds {money(cr.extraCost)} more into escrow as a new milestone.
                    </span>
                    <div className="d-flex gap-2">
          ...
          <>
            ...
            <Modal.Body>
              <p style={{ fontSize: '0.9rem' }}>
                This approves <strong>{confirming.title}</strong> and pays {order.freelancer.name} straight away.
              ...
              <div className="wm-panel" style={{ background: '#f8fafc' }}>
                ...
                <div className="d-flex justify-content-between py-1" style={{ fontSize: '0.87rem' }}>
                  <span className="text-muted">{order.freelancer.name} receives</span>
                ...
                <div className="d-flex justify-content-between py-1 border-top mt-1 pt-2" style={{ fontSize: '0.87rem' }}>
                  ...
                  <span className="wm-num" style={{ color: 'var(--amber)' }}>
                    {money(orderEscrow(order) - confirming.amount)}
                  </span>
                </div>
              </div>
            </Modal.Body>
